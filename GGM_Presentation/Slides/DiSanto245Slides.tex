...
\begin{frame}{Neighborhood Selection}
    ...
    \begin{itemize}\setlength\itemsep{4mm}
        ...
        \begin{itemize}
            \item $\exists \kappa \in (0,1), \ \max\limits_{a \in V} |\nhood(a)| = O\left(n^\kappa\right)$
...
\begin{frame}{Concluding Notes}
    \begin{itemize}\setlength\itemsep{4mm}
        \item Graphs are a powerful representation of your multivariate data {\bf(intuitively and algorithmically)}
        \item These extensions tend to distill to regularized M-estimation problems, an area with great theoretical contributions and guarantees
        \item Important to consider sparsity/degree scaling of your graph
        \begin{itemize}
            \item Early methods quickly recover graph structure, even when $N\ll d$
        \end{itemize}
        \item \texttt{huge, cglasso} packages are poweful implementations in R
    \end{itemize}
\end{frame}
...
\begin{frame}{Graphs, how and why (revisited)?}
    ...
    \node[opacity=1] (M) at (6.1, 0) {Regularized M-estimation(+)!};
    \node[opacity=1] (C) at (6.2, -0.5) {$\rightarrow$ Consistency};
    \node[opacity=1] (C) at (6.2, -1) {$\rightarrow$ Fast computation  even for $p\gg N$};
    \node[opacity=1] (MLE) at (6.2, -1.5) {$\rightarrow$ MLE via GLasso};
    \node[opacity=1] (EM) at (6.2, -2) {$\rightarrow$ EM Extensions for MAR};
