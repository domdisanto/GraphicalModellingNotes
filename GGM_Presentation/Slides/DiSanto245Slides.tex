...
\begin{frame}{Notes (To Be Deleted)}
    \begin{itemize}
        \item Add comparison of nhood/glasso for complete data 
    ...
    \begin{columns}[T] % align columns at the top
        \begin{column}{.7\textwidth}
            \begin{itemize}%{.5\textwidth}
                ...
                \begin{itemize} \item $A_{ij}=\mathbb{I}\{(i,j) \in E\}$ \end{itemize}
                \item Gaussianity gives us the nice property that $\Theta_{ij}=0 \Leftrightarrow X_i \perp X_j | X_{-\{i,j\}}$
            \end{itemize}
        \end{column}
...
\begin{frame}{Preface}
    \begin{itemize}\setlength\itemsep{6mm}
        ...
        \item Enforse sparsity for $\hat\Theta$ to account for $S$
...
\begin{itemize}
    \item True graph recovery for $N=\Omega(d^2\log p)$
    % similar error bounds give vanishing sparsity if N>p
    \item Convex pogram, quickly optimizable 
\end{itemize}
...
\begin{frame}{Simulations (Complete Data)}
    \begin{itemize}
        ...
        \item \textcolor{red}{Simulation/data set-up detailled here, banded/AR, block, and ER graph }
    \end{itemize}
\end{frame}
