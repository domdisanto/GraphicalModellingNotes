...
Recall $\Delta = |\hat{\theta_{\lambda_n}} - \theta^*|$. \newline 

Recall also that the first-order Taylor series of any loss function  $\mathcal{L}: \mathbb{R}^n \mapsto \mathbb{R}$ about $\theta^* + \Delta$ with some error $\delta \mathcal{L}(\theta^*, \Delta)$ is
