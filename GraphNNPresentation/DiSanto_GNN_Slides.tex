...
%                \item Consider random vector $X \sim F(\boldsymbol\mu, \Sigma)$ and precision matrix $\Theta \equiv \Sigma^{-1}$
                ...
                \item Can construct adjacency matrix $A \in \mathbb{R}^{|\NodeSet| \times |\NodeSet|}$ describing edge set $\EdgeSet$ 
                \begin{itemize} 
                    \item $A_{ij}=\mathbb{I}\{(i,j) \in \EdgeSet\}$ 
...
\begin{frame}{What do we estimate about graph structure?}
    ...
    \begin{gather*}
        ...
        \begin{cases}
            ...
            \\ \\ 
            \text{Graph-level level }\Phi(\Graph): \Graph %\rightarrow \mathbb{R}^m 
            \end{cases}            
    \end{gather*}
\end{frame}
...
\begin{frame}{}
    ...
    \begin{column}{0.3\textwidth}
        \begin{tikzpicture}
            \node[circle, color=red, draw] (A) at (0,0) {$X_1$};
            \node[circle, draw] (B) at (1,1) {$X_2$};
            \node[circle, color=red, draw] (C) at (1,-1) {$X_3$};
            \node[circle, draw] (D) at (2,0) {$X_4$};
    ...
    \begin{column}{0.3\textwidth}
        \begin{tikzpicture}
            \node[circle, color=black, draw] (A) at (0,0) {$X_1$};
...
\begin{itemize}
    ...
    %\item $\mathbf{D}_{ii} = \sum_{j}\mathbf{(A+I)_{ij}}$
    %\item Eigenvalue normalizing $\mathbf{D^{-1/2}(A + I)D^{-1/2}}$ for computational stability
\end{itemize}
